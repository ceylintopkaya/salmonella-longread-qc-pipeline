...
# Sample adını çıktı dosyasından çıkar
SAMPLE = os.path.basename(output_file).replace("_qc_plots_python.png", "")
...
print(f"GC Content   - Mean: {round(df['gc_content'].mean(),2)} | Median: {round(df['gc_content'].median(),2)}")
print(f"Read Length  - Mean: {round(df['length'].mean(),2)} | Median: {round(df['length'].median(),2)} | N50: {n50}")
print(f"Quality Score- Mean: {round(df['mean_quality'].mean(),2)} | Median: {round(df['mean_quality'].median(),2)}")
...
axes[0,0].legend()
stats_text = f"Mean: {round(df['length'].mean(),1)}\nMedian: {round(df['length'].median(),1)}\nN50: {n50}\nTotal: {len(df)}"
...
axes[0,1].legend()
stats_text = f"Mean: {round(df['mean_quality'].mean(),1)}\nMedian: {round(df['mean_quality'].median(),1)}\nTotal: {len(df)}"
...
axes[1,0].legend()
stats_text = f"Mean: {round(df['gc_content'].mean(),1)}%\nMedian: {round(df['gc_content'].median(),1)}%\nTotal: {len(df)}"
